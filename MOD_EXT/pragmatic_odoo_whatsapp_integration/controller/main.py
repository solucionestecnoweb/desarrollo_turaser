...
class Whatsapp(http.Controller):
    ...
    def send_notification_to_admin(self, partner, msg):
        ...
        if partner:
            channel_exist = mail_channel_obj.sudo().search([('channel_partner_ids', 'in', partner.id)], limit=1)
            if channel_exist:
                ...
                image_path = modules.get_module_resource('pragmatic_odoo_whatsapp_integration', 'static/img', 'whatsapp_logo.png')
                ...
                if len(partner_list) > 0:
                    channel = mail_channel_obj.sudo().create({
                        ...
                        'channel_partner_ids': [(6, 0, partner_list)],
                        # 'public': 'private',
                        'image_128': image,
                    })
